...
export class Motif {
  ...
  _discern(I, S) {
    ...
    const frameworkCoherence = this._calculateFrameworkCoherence(S)
    const fI = {
      speed: iC * 0.2 + 0.1,
  ...
  _environmentalFeedback(O) {
    const feedbackQuality = O.confidence * (0.8 + Math.random() * 0.4), learningSignal = Math.min(1, O.complexity * feedbackQuality)
    return {
      type: 'environmental_feedback',
      quality: feedbackQuality,
      learningPotential: learningSignal,
  ...
  _detectMotifs() {
    ...
    for (const [id, state] of Object.entries(fSs)) {
      for (const obj of state.objects) {
        if (this._isStableLoop(obj)) {
          ...
          const existingMotif = this.motifs.get(motifId)
          const motif = {
            id: motifId,
            ...
            lifespan: existingMotif ? existingMotif.lifespan + 1 : 1,
            firstDetected: existingMotif ? existingMotif.firstDetected : currentTime,
            lastUpdated: currentTime,
            interactions: obj.interactionHistory.length,
            stateHistory: [...(existingMotif?.stateHistory || []), {
              state: obj.state, energy: obj.energy, timestamp: currentTime
    ...
    for (const [motifId, motif] of this.motifs.entries()) { if (currentTime - motif.lastUpdated > this.motifTTL) this.motifs.delete(motifId) }
  }
...
Array.isArray(v)).length * 0.2))
  ...
  _calculateObjectDepth(obj, currentDepth = 0) {
    ...
    for (const value of Object.values(obj)) {
      if (typeof value === 'object' && value !== null) maxDepth = Math.max(maxDepth, this._calculateObjectDepth(value, currentDepth + 1))
    }
    return maxDepth
  }
